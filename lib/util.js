...
_.touch = function(path, mtime){
    ...
    if(mtime instanceof Date){
        ...
        light.log.error('invalid argument [mtime]');
    }
...
_.read = function(path, convert){
    ...
    if(_exists(path)){
        ...
        light.log.error('unable to read file[' + path + ']: No such file or directory.');
    }
    return content;
};
...
_.find = function(rPath, include, exclude){
    ...
    if(path){
        ...
        light.log.error('unable to find [' + rPath + ']: No such file or directory.');
    }
    return list.sort();
};
...
_.del = function(rPath, include, exclude){
    ...
    if(path) {
        if(/^(?:\w:)?\/$/.test(path)){
            light.log.error('unable to delete directory [' + rPath + '].');
        }
        var stat = fs.statSync(path);
        if(stat.isDirectory()){
        ...
        //light.log.error('unable to delete [' + rPath + ']: No such file or directory.');
    }
    return removedAll;
};
...
_.copy = function(rSource, target, include, exclude, uncover, move){
    ...
    if(source){
        ...
        light.log.error('unable to copy [' + rSource + ']: No such file or directory.');
    }
    return removedAll;
};
...
_.readJSON = function(path){
    ...
    try {
        ...
        light.log.error('parse json file[' + path + '] fail, error [' + e.message + ']');
    }
    return result;
};
...
function getFileTypeReg(type){
    ...
    if(type === 'text'){
        ...
        light.log.error('invalid file type [' + type + ']');
    }
    map = map.join('|');
